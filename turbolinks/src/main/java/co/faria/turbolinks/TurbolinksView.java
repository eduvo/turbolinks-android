...
public class TurbolinksView extends FrameLayout {
    ...
    private void init() {
        // do not add it while in design mode
        if (!isInEditMode()) {
            refreshLayout = new TurbolinksSwipeRefreshLayout(getContext(), null);
            final DisplayMetrics metrics = getResources().getDisplayMetrics();
            int mDistanceToTriggerSync = (int) (100 * metrics.density);
            refreshLayout.setDistanceToTriggerSync(mDistanceToTriggerSync);
    ...
    public void disablePullToRefresh(boolean disabled) {
        if (refreshLayout != null) {
            refreshLayout.setEnabled(!disabled);
        }
    }

    public void clearRefreshingAnimation() {
        if (refreshLayout != null) {
    ...
    boolean attachWebView(WebView webView, boolean screenshotsEnabled, boolean pullToRefreshEnabled) {
        ...
        if (webView.getParent() instanceof TurbolinksSwipeRefreshLayout) {
            ...
            if (screenshotsEnabled && (previousTurbolinksView != null)) previousTurbolinksView.screenshotView();
        ...
        TurbolinksHelper.runOnMainThread(getContext(), new Runnable() {
            @Override
            public void run() {
    ...
    public boolean hasValidScreenshotView() {
        return (screenshotView != null && screenshotOrientation == getOrientation());
    }
